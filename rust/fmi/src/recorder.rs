use std::io::Write;
use crate::{fmi2::FMU2, fmi3::FMU3, model_description::{Dimension, ModelVariable, VariableType}};
...
pub struct FMI2Recorder<'a, T: Write> {
    pub variables: Vec<&'a ModelVariable>,
    pub stream: T,
    pub fmu: &'a FMU2<'a>,
}

impl<'a, T: Write> FMI2Recorder<'a, T> {

    pub fn new(variables: Vec<&'a ModelVariable>, stream: T, fmu: &'a FMU2<'a>) -> FMI2Recorder<'a, T> {
        let mut recorder = FMI2Recorder { 
            variables,
            stream, 
            fmu, 
        };
